...
describe("solution", function(){
  ...
  assert.equal(solution.stairsIn20(stairs),54636040);
  });
});
export function stairsIn20Test(stairs) {
  let oneYrTotal = 0;
...
for (let i = 0; i < 40; i++) {
  var stairs = createArr();
  it(`Testing for ${stairs}`,function() {
  var expected = stairsIn20Test(stairs);
  assert.equal(solution.stairsIn20(stairs),expected,"It should work for random inputs too");
  }
 )};
});
