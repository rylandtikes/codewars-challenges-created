...
assert.equal(JSON.stringify(solution.nkotbVsHomie(requirements)), JSON.stringify(expected));
...
export function nkotbVsHomieTest(requirements: string[][]): string[] {
...
export function cr(a: string[], n: number): string[] {
  let r = [];
  for (let i = 0; i < n; i++) {
...
for (let i = 0; i < 40; i++) {
  ...
  it(`Testing for ${requirements}`,function() {
  let expected = nkotbVsHomieTest(requirements);
  assert.equal(JSON.stringify(solution.nkotbVsHomie(requirements)),JSON.stringify(expected),"It should work for random inputs too");
  });
 }
});
